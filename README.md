...
1. **Flèches améliorées** (`PetriCanvas.tsx`, composant `ArrowMarker`) :
   la tête de flèche est passée d'un triangle plein à une forme
   **« dart »** (triangle à dos concave, comme une pointe de fléchette),
   avec un léger contour de même couleur (`stroke` + `strokeLinejoin:
   round`) pour adoucir les angles. Rendu plus élancé et plus lisible,
   surtout quand plusieurs arcs courbes se croisent près d'un nœud. La
   taille reste fixe en unités canevas (`ARROW_SIZE`, indépendante du
   `stroke-width` de l'arc) et la pointe est toujours calée exactement sur
   le bord du nœud cible via `refX`/`refY`, comme avant.

2. **Énoncé du projet dans le JSON** : un nouveau champ `statement` a été
   ajouté au format d'export/import (voir [Format JSON](#format-json)),
   porté dans le store (`usePetriStore().statement` /
   `setStatement(...)`) et initialisé par défaut avec le texte de
   [Énoncé du projet](#énoncé-du-projet) (`PETRI_STATEMENT` dans
   `constants/petriConstants.ts`). Il est affiché en tête du panneau
   **Légende**, avant le détail place-par-place / transition-par-transition,
   pour qu'on sache toujours quel problème est modélisé — utile en
   particulier après avoir importé le réseau de quelqu'un d'autre.
   Rétro-compatible : un fichier exporté avant ce changement (sans
   `statement`) s'importe toujours normalement, avec un énoncé vide.

3. **README** (ce fichier) : documentation du projet, de son architecture
   et du format JSON complet.

> Note d'intégration : `PetriLegend.tsx` n'était pas fourni au moment de
> ces modifications. `PetriPage.tsx` lui passe désormais `statement` et
> `onStatementChange` — si votre implémentation actuelle de `PetriLegend`
> ne les gère pas encore, ajoutez un bloc en tête du panneau qui affiche
> `statement` (par ex. dans un `<pre>` ou un `<textarea>` en lecture/
> écriture relié à `onStatementChange`), avant la liste des descriptions
> P1…/T1….
